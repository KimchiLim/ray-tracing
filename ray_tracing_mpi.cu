...
__global__ void simulate_rays(float *Gdev, int *samplesdev, int nrays, int ngrid, int ntpb, int nt, int myrank) {
...
int main(int argc, char **argv) {
    ...
    ktimer.Start();
    simulate_rays<<<nblocks, ntpb, ntpb>>>(Gdev, samplesdev, nrays/nprocs, ngrid, ntpb, nt, myrank);
    ktimer.Stop();
    ...
    if (myrank > 0) {
        MPI_Send(G, ngrid*ngrid, MPI_float, 0, 0, MPI_COMM_WORLD);
        MPI_Send(&total_samples, 1, MPI_LONG_LONG_INT, 0, 1, MPI_COMM_WORLD);
    } else {
        long long samplesbuf;
        float *Gbuf = (float *)malloc(sizeof(float)*ngrid*ngrid);
        MPI_Recv(Gbuf, ngrid*ngrid, MPI_float, 1, 0, MPI_COMM_WORLD, &stat);
        MPI_Recv(&samplesbuf, 1, MPI_LONG_LONG, 1, 1, MPI_COMM_WORLD, &stat);
        for (int i = 0; i < ngrid*ngrid; i++) {
            G[i] += Gbuf[i];
        }
        total_samples += samplesbuf;
