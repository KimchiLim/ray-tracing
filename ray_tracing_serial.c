...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        printf("Usage: ./ray_tracing <nrays> <ngrid>\n");
        exit(0);
    } else {
        stop_cond = atoi(argv[1]);
        n = atoi(argv[2]);
    }
    ...
    st = clock();
    for (int _ = 0; _ < stop_cond; _++) {
        do {
            phi = ldexp(pcg32_random_r(&rng), -32) * M_PI;
            costheta = ldexp(pcg32_random_r(&rng), -32) * 2 - 1;
